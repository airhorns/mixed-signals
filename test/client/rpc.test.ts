...
describe('RPCClient', () => {
  ...
  describe('collected model references', () => {
    it('hydrates an unknown marker before resolving a call and reuses its facade', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      const result = client.call('open');
      transport.emit('R1:{"@M":"View#1"}');
      expect(transport.sent).toEqual(['M1:open:', 'M2:@M:"View#1"']);
      transport.emit('R2:[{"@M":"View#1","title":{"@S":10,"v":"Ready"}}]');
      const view = await result;
      expect(view.title.peek()).toBe('Ready');

      const again = client.call('open');
      transport.emit('R3:{"@M":"View#1"}');
      expect(await again).toBe(view);
      expect(transport.sent).toHaveLength(3);
    });

    it('accepts a method-only refresh without recursively waiting for itself', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      const result = client.call('open');
      transport.emit('R1:{"@M":"Commands#1"}');
      transport.emit('R2:[{"@M":"Commands#1"}]');
      const commands = await result;
      const ping = commands.ping();
      expect(transport.sent.at(-1)).toBe('M3:1#ping:');
      transport.emit('R3:"pong"');
      expect(await ping).toBe('pong');
    });

    it('hydrates nested and cyclic references once before publishing a root', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      transport.emit('N:@R:{"a":{"@M":"Node#a"},"alias":{"@M":"Node#a"}}');
      expect(client.root).toBeUndefined();
      transport.emit('R1:[{"@M":"Node#a","next":{"@M":"Node#b"}}]');
      await vi.waitFor(() =>
        expect(transport.sent).toContain('M2:@M:"Node#b"'),
      );
      transport.emit(
        'R2:[{"@M":"Node#b","next":{"@M":"Node#a"},"name":{"@S":1,"v":"B"}}]',
      );
      await client.ready;
      expect(client.root.a).toBe(client.root.alias);
      expect(client.root.a.next.next).toBe(client.root.a);
      expect(client.root.a.next.name.peek()).toBe('B');
      expect(transport.sent).toHaveLength(2);
    });

    it('uses complete inline models for references earlier in the same message', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      transport.emit(
        'N:@R:{"alias":{"@M":"View#1"},"view":{"@M":"View#1","title":{"@S":1,"v":"Ready"}}}',
      );
      await client.ready;
      expect(client.root.alias).toBe(client.root.view);
      expect(client.root.alias.title.peek()).toBe('Ready');
      expect(transport.sent).toEqual([]);
    });

    it('delivers hydrated notifications and subsequent updates in wire order', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      const titles: string[] = [];
      const done = new Promise<void>((resolve) =>
        client.onNotification((method, [view]) => {
          if (method === 'view') titles.push(view.title.peek());
          else if (method === 'done') resolve();
        }),
      );
      transport.emit('N:view:{"@M":"View#1"}');
      transport.emit('N:@S:10,"Updated"');
      transport.emit('N:view:{"@M":"View#1"}');
      transport.emit('N:done:');
      transport.emit('R1:[{"@M":"View#1","title":{"@S":10,"v":"Initial"}}]');
      await done;
      expect(titles).toEqual(['Initial', 'Updated']);
      expect(transport.sent).toEqual(['M1:@M:"View#1"']);
    });

    it('hydrates arguments before calling an exposed method', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      let title: string | undefined;
      client.expose({
        show(view: any) {
          title = view.title.peek();
        },
      });
      transport.emit('M7:show:{"@M":"View#1"}');
      expect(title).toBeUndefined();
      transport.emit('R1:[{"@M":"View#1","title":{"@S":10,"v":"Ready"}}]');
      await vi.waitFor(() => expect(transport.sent).toContain('R7:undefined'));
      expect(title).toBe('Ready');
    });

    it.each([
      ['E2:{"message":"Refresh denied"}', 'Refresh denied'],
      ['R2:[null]', 'Model unavailable: View#1'],
      ['R2:[]', 'Invalid model refresh response'],
    ])('rejects the original call when refresh returns %s', async (response, message) => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      const result = client.call('open');
      transport.emit('R1:{"@M":"View#1"}');
      ...
      const retry = client.call('open');
      transport.emit('R3:{"@M":"View#1"}');
      await vi.waitFor(() =>
        expect(transport.sent).toContain('M4:@M:"View#1"'),
      );
      transport.emit('R4:[{"@M":"View#1","title":{"@S":10,"v":"Ready"}}]');
      expect((await retry).title.peek()).toBe('Ready');
    });

    it('rejects readiness when a root reference cannot be refreshed', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      const ready = expect(client.ready).rejects.toThrow('Missing root');
      transport.emit('N:@R:{"@M":"Root#1"}');
      transport.emit('E1:{"message":"Missing root"}');
      await ready;
      expect(client.root).toBeUndefined();
    });

    it('rejects pending hydration on disconnect and ignores late snapshots', async () => {
      const transport = new FakeTransport();
      const client = new RPCClient(transport);
      transport.emit('N:@R:{}');
      const result = client.call('open');
      transport.emit('R1:{"@M":"View#1"}');
      transport.close(new Error('Disconnected'));
      await expect(result).rejects.toThrow('Disconnected');
      transport.emit('R2:[{"@M":"View#1","title":{"@S":10,"v":"Old"}}]');
      const replacement = new FakeTransport();
      client.reconnect(replacement);
      replacement.emit('N:@R:{}');
      await client.ready;
      const retry = client.call('open');
      replacement.emit('R3:{"@M":"View#1"}');
      expect(replacement.sent).toContain('M4:@M:"View#1"');
      replacement.emit('R4:[{"@M":"View#1","title":{"@S":10,"v":"New"}}]');
      expect((await retry).title.peek()).toBe('New');
    });
  });
